...
（湖北省内电力/能源新闻。如无则写"暂无湖北本地相关新闻，持续监控中。"）

四、AI + 电力（N条）

（AI与电力/能源结合。如无则写"暂无相关动态。"）
...
| 1#铜均价 | XX,XXX元/吨 |
| 涨跌 | ±X,XXX元/吨 |
| 价格区间 | XX,XXX-XX,XXX元/吨 |
| 铜升贴水 | XXX元/吨 |
| 日期 | MM-DD |

👉 判断：一行市场判断+采购建议。
（10:30前无数据则写"暂无今日铜价数据，开盘后更新。"）
...
早报完成时间：YYYY-MM-DD HH:mm
```

**戴总认可格式规则（2026-04-08 确认，以此为准）：**
- **精简无干扰**：每条新闻 = 【标题】+ 一句话说明 + 👉 一行洞察，不要来源/日期/链接堆砌
- **编号连续**：全文从1开始连续编号，不按板块重新编号
- **每板块标注条数**：如「一、今日最重要（3条）」
- **👉 统一格式**：板块一二三四用「影响/价值」，板块五用「判断」，板块六用「本周关注」
- **铜价用表格**：| 指标 | 数据 | 的 markdown 表格格式
- **结尾时间戳**：早报完成时间：YYYY-MM-DD HH:mm
- **无 emoji 装饰**：除了👉之外不用其他 emoji，不用✅🔸📊📎等
- **总条数 ≥ 8 条**，每个板块必须存在
